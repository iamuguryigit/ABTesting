...
class AbTesting_Binary(AbTesting):

    def __init__(self, data: pd.DataFrame(), variant_col: str = "VARIANT_NAME", target_col: str = "RETENTION"):

        super().__init__(data, variant_col, target_col)

        self.related_data = self.summary_data[["mean", "count"]].reset_index()

    def statistical_calculations(self):

        self.related_data["std"] = np.sqrt(self.related_data["mean"] * (1 - self.related_data["mean"]))
    ...
    def ab_conf_interval(self, alpha=0.05, two_tailed=False, power=0.8, iter_count=10000):
        ...
        SE, pop_std = self.statistical_calculations()
